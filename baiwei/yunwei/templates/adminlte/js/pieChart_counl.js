...
function pieChart() {
  ...
  function init() {
    ...
    canvas = document.getElementById('chart_counl');
    ...
    $('#chartData_counl td').each( function() {
      currentCell++;
      if ( currentCell % 2 != 0 ) {
        currentRow++;
    ...
    drawChart();
    $('#chart_counl').click ( handleChartClick );
  }
  ...
  function startPullOut ( slice ) {
    ...
    $('#chartData_counl td').removeClass('highlight');
    var labelCell = $('#chartData_counl td:eq(' + (slice*2) + ')');
    var valueCell = $('#chartData_counl td:eq(' + (slice*2+1) + ')');
