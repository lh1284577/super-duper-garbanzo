...
function pieChart() {
  ...
  function init() {
    ...
    canvas = document.getElementById('chart_reqs');
    ...
    $('#chartData_reqs td').each( function() {
      currentCell++;
      if ( currentCell % 2 != 0 ) {
        currentRow++;
    ...
    drawChart();
    $('#chart_reqs').click ( handleChartClick );
  }
  ...
  function startPullOut ( slice ) {
    ...
    $('#chartData_reqs td').removeClass('highlight');
    var labelCell = $('#chartData_reqs td:eq(' + (slice*2) + ')');
    var valueCell = $('#chartData_reqs td:eq(' + (slice*2+1) + ')');
