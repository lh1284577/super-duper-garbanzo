...
function pieChart() {
  ...
  function init() {
    ...
    canvas = document.getElementById('chart_logon');
    ...
    $('#chartData_logon td').each( function() {
      currentCell++;
      if ( currentCell % 2 != 0 ) {
        currentRow++;
    ...
    drawChart();
    $('#chart_logon').click ( handleChartClick );
  }
  ...
  function startPullOut ( slice ) {
    ...
    $('#chartData_logon td').removeClass('highlight');
    var labelCell = $('#chartData_logon td:eq(' + (slice*2) + ')');
    var valueCell = $('#chartData_logon td:eq(' + (slice*2+1) + ')');
