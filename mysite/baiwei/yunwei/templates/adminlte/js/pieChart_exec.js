...
function pieChart() {
  ...
  function init() {
    ...
    canvas = document.getElementById('chart_exec');
    ...
    $('#chartData_exec td').each( function() {
      currentCell++;
      if ( currentCell % 2 != 0 ) {
        currentRow++;
    ...
    drawChart();
    $('#chart_exec').click ( handleChartClick );
  }
  ...
  function startPullOut ( slice ) {
    ...
    $('#chartData_exec td').removeClass('highlight');
    var labelCell = $('#chartData_exec td:eq(' + (slice*2) + ')');
    var valueCell = $('#chartData_exec td:eq(' + (slice*2+1) + ')');
